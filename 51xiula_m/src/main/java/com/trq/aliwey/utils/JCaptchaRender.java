...
import com.jranuan.kit.EncryptionKit;
import com.jranuan.kit.StrKit;
import com.jranuan.render.Render;

/**
 * 验证码Render
 * @title JCaptchaRender.java
 * @description 
 * @company 北京思维夫网络科技有限公司
 * @url http://www.meitimao.com
 * @author 卢春梦
 * @version 1.0
 * @created 2015年5月18日上午11:06:49
 */
public class JCaptchaRender extends Render {
	...
	// 默认的验证码大小
	private static final int WIDTH = 180, HEIGHT = 40;
	// 验证码随机字符数组
	private static final String[] strArr = {"3", "4", "5", "6", "7", "8", "9", "A", "B", "C", "D", "E", "F", "G", "H", "J", "K", "M", "N", "P", "Q", "R", "S", "T", "U", "V", "W", "X", "Y"};
	// 验证码字体
	private static final Font[] RANDOM_FONT = new Font[] {
		new Font("nyala", Font.BOLD, 36),
		new Font("Arial", Font.BOLD, 38),
		new Font("Bell MT", Font.BOLD, 37),
		new Font("Credit valley", Font.BOLD, 38),
		new Font("Impact", Font.BOLD, 36),
	...
	public static boolean validate(Controller controller, String inputRandomCode) {
		if (StrKit.isBlank(inputRandomCode))
			return false;
		try {
			inputRandomCode =  EncryptionKit.md5Encrypt(inputRandomCode.toLowerCase());
			// 判定是否验证通过
