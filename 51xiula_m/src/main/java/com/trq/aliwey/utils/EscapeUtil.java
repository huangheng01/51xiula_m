...
public class EscapeUtil {
	...
	/**
	 * 转码一组参数， 如果是一个参数值是一个，转换结果为 single=value 如果参数值是多个，转换结果为
	 * multi=value1&multi=value2
	 * 
	 * @param paramMap
	 * @param encode
	 * @return
			...
			for (Object o : paramMap.entrySet()) {
				...
				if (value != null && value.getClass().isArray()) {
					for (Object each : ((Object[]) value)) {
						...
						sb.append('=');
						sb.append(URLEncoder.encode(StringUtil.toString(each), encode));
						sb.append('&');
					}
				} else {
					sb.append(entry.getKey());
					sb.append('=');
					sb.append(URLEncoder.encode(StringUtil.toString(value), encode));
					sb.append('&');
				}
			}
			if (sb.length() > 0) {
	...
	public static String encodeURL(String url, String encode) {
		try {
			...
			for (int index = 0; index < ss.length; index++) {
				ss[index] = URLEncoder.encode(ss[index], encode);
			}
	...
	public static String decodeParameter(String content, String encode) {
		try {
